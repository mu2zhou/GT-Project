...
# ----------------------------------------------------------
#  ENU helper : (lat,lon) in rad  →  (x,y) in m
#  We re-use the same reference point that is already used
#  in the file (first epoch of the first vehicle)
# ----------------------------------------------------------
def _enu_from_rad(lat_rad, lon_rad, ref_lat_rad, ref_lon_rad):
    """very fast local ENU approximation (deg ≪ 1)"""
...
def OpenACC_convert_format(input_path, output_path, id_map):
    # --- read meta line & data ---------------------------------
    meta = pd.read_csv(input_path, nrows=1, header=None).iloc[0, 1:].dropna()
    vehicle_ids = [id_map.get(str(x), str(x)) for x in meta]
    n_veh = len(vehicle_ids)

    raw = pd.read_csv(input_path, skiprows=5)
    raw['Time'] -= raw['Time'].iloc[0]          # make time start at 0

    # --- build uniform table ----------------------------------
    rows = []
    # for i in range(n_veh - 1):
    # 只有 IVS0 存在 → 只处理 1→2 这一对
    for i in range(int((n_veh - 1))):   # 现在 n_veh=2 → range(1) → 只跑 i=0
        if f'IVS{i+1}' not in raw.columns:   # 保险
            continue
        # pick leader / follower columns
        # 新代码（OpenACC 列名从 1 开始）
        lat_l = raw[f'Lat{i+1}']          # 领头车
        lon_l = raw[f'Lon{i+1}']
        lat_f = raw[f'Lat{i+2}']
        lon_f = raw[f'Lon{i+2}']
        spd_l = raw[f'Speed{i+1}']
        spd_f = raw[f'Speed{i+2}']
        ivs   = raw[f'IVS{i+1}']
        drv_f = raw.get(f'Driver{i+1}', 'ACC')

        # reference point = first epoch of leader
        ref_lat = lat_l.iloc[0]
        ref_lon = lon_l.iloc[0]

        # convert to local metres
        xl, yl = _enu_from_rad(lat_l.values, lon_l.values, ref_lat, ref_lon)
        xf, yf = _enu_from_rad(lat_f.values, lon_f.values, ref_lat, ref_lon)

        # 先构造一个临时 DataFrame，再用布尔索引过滤
        tmp_df = pd.DataFrame({
            'Time': raw['Time'],
            'xl': xl,
            'yl': yl,
        ...
        tmp_df = tmp_df[tmp_df['mask']].reset_index(drop=True)

        for t, xl_t, yl_t, xf_t, yf_t, sl, sf, gap in zip(
                tmp_df['Time'], tmp_df['xl'], tmp_df['yl'],
                tmp_df['xf'], tmp_df['yf'], tmp_df['sl'], tmp_df['sf'], tmp_df['gap']):
            rows.append({
                'Trajectory_ID': i,
                'Time_Index': t,
                'ID_LV': vehicle_ids[i],
                'Type_LV': 0,
                'Pos_LV': 0.0,            # will be filled later
                'Speed_LV': sl,
                'Acc_LV': 0.0,
                'ID_FAV': vehicle_ids[i+1],
                'Pos_FAV': 0.0,
                'Speed_FAV': sf,
                'Acc_FAV': 0.0,
                'Spatial_Gap': gap,
                'Spatial_Headway': gap + default_vehicle_length,
                'Speed_Diff': sl - sf,
                # >>> NEW : real BEV coordinates <<<
                'leader_x': xl_t,
                'leader_y': yl_t,
                'follower_x': xf_t,
                'follower_y': yf_t,
                'leader_length': default_vehicle_length,
                'follower_length': default_vehicle_length
            })
    ...
    # --- derive longitudinal positions (same logic as before) ---
    data['Acc_LV']  = data['Speed_LV'].diff() / 0.1
    data['Acc_FAV'] = data['Speed_FAV'].diff() / 0.1
    data['Pos_FAV'] = (data['Speed_FAV'].rolling(2).mean() * 0.1).cumsum()
