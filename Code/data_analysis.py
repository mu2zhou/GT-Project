...
# ---------------------------- 新增 BEV 轨迹可视化 ----------------------------
def draw_BEV_trajectory(step3_csv_path: str, scene_name: str, out_dir: str):
    import os
    import matplotlib.pyplot as plt
    import seaborn as sns

    df = pd.read_csv(step3_csv_path)
    ...
    # 2. 先拿轨迹 ID
    traj_ids = df['Trajectory_ID'].unique()[:50]

    # 3. 再生成调色板（必须在 for 之前）
    palette = sns.color_palette("husl", len(traj_ids))

    # 4. 开始画图
    for idx, tid in enumerate(traj_ids):
        sub_full = df[df['Trajectory_ID'] == tid].sort_values('Time_Index')
        sub = sub_full.iloc[::10]          # 降采样
        plt.plot(sub['Time_Index'], sub['Pos_LV'], color=palette[idx], lw=1, label=f'LV-{tid}')
        plt.plot(sub['Time_Index'], sub['Pos_FAV'], color=palette[idx], lw=1, ls='--', label=f'FAV-{idx}')

    # 5. 标签 & 保存
    plt.xlabel('Time (s)', fontsize=14)
    plt.ylabel('Longitudinal position (m)', fontsize=14)
    plt.title(f'BEV Trajectory Overview – {scene_name}', fontsize=16)
    plt.tight_layout()
    os.makedirs(out_dir, exist_ok=True)
    save_path = os.path.join(out_dir, f'BEV_{scene_name}.png')
    plt.savefig(save_path, dpi=300)
    plt.close()
    print(f'BEV 轨迹图已保存：{save_path}')